...
bool c_network_session::acknowledge_join_request(s_transport_address const* address, e_network_join_refuse_reason reason)
{
    ...
    printf("MP/NET/SESSION,CTRL: c_network_session::acknowledge_join_request: [%s] join failed, sending refusal (%s) to '%s'", managed_session_index_string, join_refuse_reason_string, transport_address_string);
    s_network_message_leave_acknowledge message;
    managed_session_get_id(this->m_managed_session_index, &message.session_id);
    return this->m_message_gateway->send_message_directed(address, _network_message_type_leave_acknowledge, 20, &message);
}
...
char c_network_session::handle_join_request(s_transport_address const* address, s_network_message_join_request const* message)
{
    // temporary - TODO
    // log the packet send
    this->acknowledge_join_request(address, _network_join_refuse_reason_session_full);
    return 1;
}