...
void anvil_dedi_test_set_player_data(c_network_session_membership* membership)
{
    ...
    for (long i = membership->get_first_player(); i != -1; i = membership->get_next_player(i))
    {
        ...
        memcpy(&host_configuration->player_appearance.service_tag, service_tag1, 10);
        ...
        host_configuration->s3d_player_appearance.loadouts[0].armor = _armor_pilot;
        host_configuration->s3d_player_appearance.loadouts[0].primary_weapon = _smg_v5;
        host_configuration->s3d_player_appearance.loadouts[0].secondary_weapon = _magnum_v1;
        host_configuration->s3d_player_appearance.loadouts[0].tactical_packs[0] = _adrenaline;
        ...
        if (current_player->peer_index == membership->host_peer_index())
        {
            ...
            host_configuration->s3d_player_appearance.loadouts[0].armor = _armor_scout;
