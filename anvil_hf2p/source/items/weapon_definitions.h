...
struct _weapon_definition
{
	...
	// new since ms23
	s_tag_block new_first_person_weapon_offsets;
...
static_assert(sizeof(_weapon_definition) == 0x390);
...
static_assert(0x364 == OFFSETOF(_weapon_definition, new_first_person_weapon_offsets));
static_assert(0x38C == OFFSETOF(_weapon_definition, unit_stow_anchor_name));

struct weapon_definition
{
	static tag const k_group_tag = WEAPON_TAG;

	_object_definition object; // 0x120
	_item_definition item; // 0xB4
	_weapon_definition weapon; // offset 0x1D4
};
static_assert(sizeof(weapon_definition) == 0x564);