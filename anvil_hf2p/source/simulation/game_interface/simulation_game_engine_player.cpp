...
void simulation_action_game_engine_player_update(short player_index, c_simulation_object_update_flags& update_flags)
{
...
void simulation_action_game_engine_player_update(datum_index player_index, e_simulation_player_update_flag update_flag)
{
	...
	c_simulation_object_update_flags flags(player_index, update_flag);
	simulation_action_game_engine_player_update(absolute_player_index, flags);
}
...
bool __stdcall c_simulation_player_respawn_request_event_definition__apply_game_event(long reference_gamestate_count, const datum_index* gamestate_indicies, long payload_size, const s_player_respawn_target_request_event_data* payload)
{
	...
	if (spectating_player_absolute_index != 0xFFFF && spectating_player_absolute_index >= k_maximum_players)
	{
		return false;
	}
	...
	player_data->multiplayer.dead_camera_target_player_index = player_index_from_absolute_player_index(spectating_player_absolute_index);
