...
long c_simulation_entity_database::entity_create(e_simulation_entity_type entity_type)
{
	...
	if (entity_allocate_creation_data(entity_type, &creation_data_size, &creation_data) && entity_allocate_state_data(entity_type, &state_data_size, &state_data))
	{
...
void c_simulation_entity_database::entity_update(long entity_index, c_simulation_object_update_flags& update_mask, bool force_mask_update)
{
	s_simulation_entity* entity = entity_get(entity_index);
...
void c_simulation_entity_database::entity_delete_internal(long entity_index)
{
	...
	if (m_resetting)
	{
		long absolute_index = DATUM_INDEX_TO_ABSOLUTE_INDEX(entity_index);
		ASSERT(absolute_index >= 0 && absolute_index < NUMBEROF(m_entity_data));
		entity = &m_entity_data[absolute_index];
	}
	else
	{
		entity = entity_get(entity_index);
		ASSERT(m_entity_manager);
		ASSERT(m_entity_manager->is_entity_allocated(entity_index));
		ASSERT(m_entity_manager->is_entity_being_deleted(entity_index));
		ASSERT(!entity->exists_in_gameworld);
