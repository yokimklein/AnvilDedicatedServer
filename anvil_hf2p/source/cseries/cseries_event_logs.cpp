...
void c_file_output_buffer::flush()
{
	if (m_buffer_length > 0)
	{
		...
		if (m_file)
		{
			file_write(m_file, (ulong)m_buffer_length, m_buffer);
		}
		m_buffer_length = 0;
	}
}
...
void c_file_output_buffer::write(long buffer_size, const void* buffer)
{
	if (m_file)
	{
...
s_event_log* event_log_get(long event_log_index)
{
...
long event_log_new(const char* event_log_name, c_event_log_flags event_log_flags)
{
	long event_log_index = NONE;
...
void event_logs_close()
{
	...
	for (long event_log_index = 0; event_log_index < event_log_globals.event_log_count; event_log_index++)
	{
...
bool copy_file_to_error_snapshot(const s_file_reference* source_file_reference, s_file_reference* file_reference_out)
{
	...
	constexpr long name_flags = FLAG(_name_parent_directory_bit) | FLAG(_name_file_bit) | FLAG(_name_extension_bit);
...
long upload_error_file_start(long maximum_file_count, s_file_reference* out_file_list)
{
	c_critical_section_scope section_scope(k_crit_section_event_logs);
...
s_file_reference* acquire_report_file_reference(long event_log_index)
{
	s_event_log* event_log = event_log_get(event_log_index);
...
void release_report_file_reference(long event_log_index, bool close_reference)
{
	s_event_log* event_log = event_log_get(event_log_index);
...
void flush_event_log_cache()
{
	...
	{
		ulong flush_log_start_time = network_time_get();
		...
		long entry_count = g_event_log_cache.entry_stack.count();
		for (long entry_index = 0; entry_index < entry_count; entry_index++)
		{
			s_event_log_cache_entry* entry = g_event_log_cache.entry_stack.get(entry_index);
			for (long event_log_index = 0; event_log_index < 32; event_log_index++)
			{
		...
		{
			event(_event_error, "events: flush_event_log_cache took %d msecs to flush", network_time_since(flush_log_start_time));
		}
	}
}

void write_raw_to_error_file(c_file_output_buffer* file_buffer, long event_log_index, const char* string, bool flush)
{
	if (!events_force_no_log)
	{
		...
		if (file_buffer)
		{
			file_buffer->write((ulong)strlen_debug(string), string);
		}
		else if (s_file_reference* reference = acquire_report_file_reference(event_log_index))
		{
			file_write(reference, (ulong)strlen_debug(string), string);
			release_report_file_reference(event_log_index, flush);
		}
	}
}

void write_event_log_cache_entry(bool use_report_buffers, long event_log_index, const char* string, bool flush)
{
...
void write_to_event_log(const long* event_log_indices, long event_log_count, const char* string)
{
...
void write_to_event_log_cache(const long* event_log_indices, long event_log_count, const char* string)
{
	...
	long string_size = (long)strlen_debug(string) + 1;
	...
	if (event_log_globals.cache_event_log_output
		&& g_event_log_cache.entry_string_cache_size + string_size <= k_entry_string_cache_maximum_size)
	{
		c_flags<long, ulong, 32> category;
		long entry_string_cache_size = g_event_log_cache.entry_string_cache_size;

		g_event_log_cache.entry_stack.push();
		s_event_log_cache_entry* entry_string = g_event_log_cache.entry_stack.get_top();

		for (long event_log_num = 0; event_log_num < event_log_count; event_log_num++)
		{
			long event_log_index = event_log_indices[event_log_num];
