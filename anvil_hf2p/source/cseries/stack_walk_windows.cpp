...
const char* symbol_name_from_address(ulong routine_address, long* out_symbol_offset)
{
	...
	for (ulong module_index = 0; module_index < (cbNeeded / sizeof(HMODULE)); module_index++)
	{
		...
		if (!IN_RANGE_INCLUSIVE(routine_address, (ulong)module_info.lpBaseOfDll, (ulong)module_info.lpBaseOfDll + module_info.SizeOfImage))
		{
			continue;
		}
...
void walk_stack(ulong* routine_addresses, ulong number_of_levels, short levels_to_ignore, long* levels_dumped)
{
	levels_to_ignore++;
	...
	csmemcpy(routine_addresses, stack, sizeof(ulong) * number_of_levels);
	*levels_dumped = stack_trace_result;
}

void stack_walk_with_context_internal(const s_file_reference* file, short levels_to_ignore, _CONTEXT* context, long number_of_levels, ulong* routine_addresses, long* out_levels_dumped)
{
	levels_to_ignore++;
...
void stack_walk_print(s_file_reference* file, _CONTEXT* context, long levels_dumped, const ulong* routine_addresses)
{
	for (long i = levels_dumped - 1; i >= 0; i--)
	{
		ulong routine_address = routine_addresses[i];
