...
#include "main/debug_keys.h"
#include "main/main.h"
#include "main/main_game.h"
#include "multithreading/threads.h"
#include "networking/tools/remote_command.h"
...
void __cdecl console_initialize()
{
	...
	if (initialize_console)
	{
		...
		console_globals.input_state.prompt.set("halo( ");
	...
	PROFILER(console_update)
	{
		if (console_is_active())
		{
			for (long key_index = 0; key_index < console_globals.input_state.key_count; key_index++)
			{
				...
				if (key->key_type == _key_type_down && (key->ascii_code == _key_tilde || key->ascii_code == _key_f1))
				{
					console_close();
					break;
				}
...
bool string_to_real(const char* string, real* value)
{
	if (!string)
	{
		return true;
...
bool string_to_short_integer(const char* string, short* value)
{
	if (!string)
	{
		return true;
...
bool string_to_long_integer(const char* string, long* value)
{
	if (!string)
	{
		return true;
...
callback_result_t set_callback(const void* userdata, long token_count, tokens_t const tokens)
{
...
void status_lines_initialize_simple(s_status_line* status_line, bool* flag, const char* identifier, long count)
{
...
bool string_cache_add(s_string_cache* cache, const char* string, real alpha, const real_rgb_color& color, e_text_justification justification)
{
	bool result = false;
