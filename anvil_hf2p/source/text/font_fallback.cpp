...
bool fallback_font_get_character(e_utf32 utf_character, const s_font_character** out_character, const void** out_pixel_data)
{
	...
	ASSERT(g_fallback_font_header->location_table_offset + g_fallback_font_header->location_table_count * sizeof(ulong) <= sizeof(k_fallback_font_data));

	if (VALID_INDEX((long)utf_character, g_fallback_font_header->location_table_count))
	{
		ulong location = *(ulong*)offset_pointer(k_fallback_font_data, g_fallback_font_header->location_table_offset + sizeof(ulong) * utf_character);
		if (location != NONE)
		{
			ulong page_index = location & MASK(21);
			ulong page_count = location >> 21;
			ulong character_offset = g_fallback_font_header->character_data_offset + k_font_character_page_size * page_index;
...
void fallback_font_initialize()
{
	if (!g_fallback_font_header)
	{
		...
		ulong* location_table = (ulong*)offset_pointer(header, header->location_table_offset);
		ASSERT(header->location_table_offset >= 0 && header->location_table_offset + header->location_table_count * sizeof(ulong) <= sizeof(k_fallback_font_data));
		...
		for (long i = 0; i < location_table_count; ++location_table)
		{
			...
			{
				ulong page_index = *location_table & MASK(21);
				ulong page_count = *location_table >> 21;
	
				ulong character_offset = header->character_data_offset + k_font_character_page_size * page_index;
				ASSERT(page_count > 0 && page_count <= k_font_character_maximum_pages);
